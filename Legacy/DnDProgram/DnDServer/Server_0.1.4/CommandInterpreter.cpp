...
void CommandInterpretor::Interpret(std::string command)
{
	...
	if (result != Result::Error)
	{
		...
		else if (parsedCommand[0] == "sync") { parsedCommand.erase(parsedCommand.begin()); Sync(parsedCommand); }
		else if (parsedCommand[0] == "spawn") { parsedCommand.erase(parsedCommand.begin()); Spawn(parsedCommand); }
		else if (parsedCommand[0] == "center") { parsedCommand.erase(parsedCommand.begin()); Center(parsedCommand); }
		else
		{
...
{
	if (parameter.size() >= 2 && parameter[0] == "f")
	{
...
{
	DnD::Player *player = Program->GraphicWin.GameWorld->RemovePlayer(parameter[0]);

	if (player)
	{
		for (int i = 0; i < (int)Program->Network->EventTotal; i++)
		{
			if (Program->Network->SocketArray[i].Player == player)
			{
				//closesocket(Program->Network->SocketArray[i].Socket);
				Program->Network->FreeSocketInformation(i);
			}
		}
		delete player;
...
void CommandInterpretor::Sync(std::vector<std::string> parameters)
{
	if (parameters.size() && parameters[0] == "force")
	{
		Program->Network->UpdateWorldState(true);
	}
	else
	{
		Program->Network->UpdateWorldState();
	}
}

void CommandInterpretor::Spawn(std::vector<std::string> parameters)
{
	if (parameters.size())
	{
		DnD::Player *player = new DnD::Player();
		player->Name = parameters[0];
		Program->GraphicWin.GameWorld->PlayerList.push_back(player);
		Program->ConsoleWin.CommandLine->Write("Created New Player:" + player->Name + "\n");
		Program->GraphicWin.WindowPtr->AddMessage(MessageData({ WL_UPDATESURFACE, 0, 0 }), true);
	}
}

void CommandInterpretor::Center(std::vector<std::string> parameters)
{
	Program->GraphicWin.GameWorld->ViewPos = { 0,0 };
	Program->GraphicWin.GameWorld->Scale = 1;
	Program->GraphicWin.WindowPtr->AddMessage(MessageData({ WL_UPDATESURFACE, 0, 0 }), true);
}
