...
DnD::DnDProgram::DnDProgram(HINSTANCE hInstance, INT iCmdShow)
{
	...
	GraphicWin.SelectPlayer(0); GraphicWin.ActivePlayer = GraphicWin.GameWorld->PlayerList[0];
...
void DnD::DnDProgram::GraphicWindow::Callback(MessageData message, DnDProgram *program)
{
	...
	if (message == WL_UPDATESURFACE)
	{
		...
		GameWorld->Draw(SelectedEnvironment, SelectedPlayerIndex ? ActivePlayer : 0);//pass 0 to signify draw all
		...
		if (SelectedTool == SubSelect && SelectedObject)
		{
			if (message == WM_LBUTTONDOWN && mousePos.X < 800)
			{
				...
				if (SelectedObject->Type == EllipseObject)
				{
					...
					//if (Rect(GameWorld->Transform().ToInt2() - 3, GameWorld->Transform(line->LineList[i]).ToInt2() + 3).Intersect(mousePos))
				}
			}
			if (message == WM_MOUSEMOVE && Drag)
			{
				if (SelectedObject->Type == LineObject)
				{
					...
					line->LineList[SubSelectIndex] = InitialObjectPos[0] + (mouseTransform.ToInt2() - InitialMousePos);
				}
				WindowPtr->AddMessage(MessageData({ WL_UPDATESURFACE, 0, 0 }), true);
			}
		}
...
void DnD::DnDProgram::GraphicWindow::DrawSelectMenu(DnDProgram *program)
{
	...
	InterfaceSurface->DrawRectangle({ { 182, 8 },{ 190, 18 } }, Colour{ 255,255,255 });//delete
	{
		...
		for (int i = max(PlayerScroll / 25, 0); i < min(PlayerScroll / 25 + 7, GameWorld->PlayerList.size()); i++)
		{
			...
			if (GameWorld->PlayerList[i]->Connected)
			{
				InterfaceSurface->FillRectangle({ { 35, 20 + i * 25 - PlayerScroll },{ 45, 20 + 24 + i * 25 - PlayerScroll } }, { 255,255,255 });
			}
			else
			{
				InterfaceSurface->FillRectangle({ { 35, 20 + i * 25 - PlayerScroll },{ 45, 20 + 24 + i * 25 - PlayerScroll } }, { 50,50,50 });
			}
			//InterfaceSurface->DrawText({ 30, 160 + 8 + i * 25 - PlayerScroll }, "Name____________.0000", Font("consolas", 10, { 255,255,255 }));
			InterfaceSurface->DrawText({ 50, 20 + 4 + i * 25 - PlayerScroll }, GameWorld->PlayerList[i]->Name, Font("consolas", 10, { 255,255,255 }));
		}
...
void DnD::DnDProgram::GraphicWindow::SelectObject(int index)
{
	SelectObject(SelectedEnvironmentIndex, SelectedLayerIndex, index);
}
...
void DnD::DnDProgram::GraphicWindow::SubSelectObject(int index)
{
	SubSelectObject(SelectedEnvironmentIndex, SelectedLayerIndex, index);
}
...
void DnD::DnDProgram::GraphicWindow::SetObjectSelected(int index)
{
	SetObjectSelected(SelectedEnvironmentIndex, SelectedLayerIndex, index);
}
...
void DnD::DnDProgram::GraphicWindow::DeselectObject(int index)
{
	DeselectObject(SelectedEnvironmentIndex, SelectedLayerIndex, index);
}

void DnD::DnDProgram::GraphicWindow::SelectLayer(int eIndex, int lIndex)
{
	SelectedLayer = GameWorld->EnvironmentList[eIndex]->LayerList[lIndex];
...
void DnD::DnDProgram::GraphicWindow::SelectLayer(int index)
{
	SelectLayer(SelectedEnvironmentIndex, index);
}
...
void DnD::DnDProgram::GraphicWindow::SetLayerSelected(int index)
{
	SetLayerSelected(SelectedEnvironmentIndex, index);
}
...
void DnD::DnDProgram::GraphicWindow::DeselectLayer(int index)
{
	DeselectLayer(SelectedEnvironmentIndex, index);
}

void DnD::DnDProgram::GraphicWindow::SelectEnvironment(int index)
{
	SelectedEnvironment = GameWorld->EnvironmentList[index];
	SelectedEnvironment->Selected = Selected;
	SelectedEnvironmentIndex = index;

	GameWorld->DeselectLayers();
	//if (SelectedEnvironment->LayerList.size())
	//{
	SelectLayer(0);
	SelectedLayer = SelectedEnvironment->LayerList[0];
...
void DnD::DnDProgram::GraphicWindow::SetEnvironmentSelected(int index)
{
	GameWorld->EnvironmentList[index]->Selected = Selected;
}

void DnD::DnDProgram::GraphicWindow::DeselectEnvironment(int index)
{
	GameWorld->EnvironmentList[index]->Selected = Unselected;
	if (GameWorld->EnvironmentList[index] == SelectedEnvironment)
	{
...
void DnD::DnDProgram::Save(std::string name)
{
	...
	*(int *)buf = (int)GraphicWin.GameWorld->PlayerList.size();
	...
	for (int p = 1; p < (int)GraphicWin.GameWorld->PlayerList.size(); p++)
	{
		...
		file.write((char *)&player->Visible, sizeof(bool)); file << "\n";
...
void DnD::DnDProgram::Load(std::string name)
{
	...
	GraphicWin.SelectedPlayer = 0; GraphicWin.ActivePlayer = 0; GraphicWin.SelectedPlayerIndex = 0;
