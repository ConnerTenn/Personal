...
void Container::UpdateMsg()
{
	...
	if (input->MouseState.Click)
	{
		...
		{
			State.Focus = true;
			if (Parent) { Parent->State.Focus = false; }
		}
		else
		{
			State.Focus = false;
...
{
	/*
	Hfnt = CreateFont(font.FontSize, font.FontSize / font.HeightWidthRatio, 0, 0, (font.Style & 0b0001) == 0b0001 ? FW_BOLD : FW_NORMAL, font.Style & 0b0010, font.Style & 0b0100, font.Style & 0b1000, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_CHARACTER_PRECIS, ANTIALIASED_QUALITY, DEFAULT_PITCH, TEXT(font.Typeface.c_str()));
	SelectObject(Hdc, Hfnt);
	
	SetTextColor(Hdc, RGB(font.FontColour.R, font.FontColour.G, font.FontColour.B));
	SetBkMode(Hdc, TRANSPARENT);
	...
	//*/
	///*
	GdiFont = new Gdiplus::Font(std::wstring(font.Typeface.begin(), font.Typeface.end()).c_str(), (Gdiplus::REAL)font.FontSize, (INT)font.Style);
	SolidBrush->SetColor(Gdiplus::Color(font.FontColour.A, font.FontColour.R, font.FontColour.G, font.FontColour.B));
	Graphics->DrawString(std::wstring(text.begin(), text.end()).c_str(), (INT)text.length(), GdiFont, Gdiplus::PointF((Gdiplus::REAL)x, (Gdiplus::REAL)y), SolidBrush);
	delete GdiFont; GdiFont = 0;
	//*/
}
