...
LRESULT CALLBACK Window::WndProc(HWND Hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	{
		//add items to queue
		pThis->MessageQueue.push({ message, wParam, lParam });
		
		//unlock callback thread thread
		std::lock_guard<std::mutex> lock(pThis->CallbackMutex);
		pThis->CallbackConditionVariable.notify_one();
		pThis->CallbackReady = true;
	}
...
void Window::WindowCallback(Window *win)
{
	while (!win->State.Quit)
	{
		std::unique_lock<std::mutex> lock(win->CallbackMutex);
		win->CallbackConditionVariable.wait(lock, [&]{ return win->CallbackReady; });
		
		while (!win->MessageQueue.empty() && !win->State.Quit)
		{
			//process message
			CallbackData message = win->MessageQueue.front();
			win->MessageQueue.pop();



			if (win->Callback) { win->Callback(message); }
		}

		win->CallbackReady = false;
	}
}

Window::Window(int width, int height, HINSTANCE hinstance, int iCmdShow, void(*callback)(CallbackData message))//:
																		//Input(this)
{
	Width = width;
	Height = height;
	...
	Callback = callback;
	CallbackThread = std::thread(WindowCallback, this);
}
...
Container *Window::CreateContainer(int x, int y, int width, int height, int frameNum)
{
	Container *container = new Container(x, y, width, height, NULL, this, frameNum);
