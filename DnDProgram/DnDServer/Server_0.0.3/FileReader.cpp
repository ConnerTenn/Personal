...
Result FileReader::ParseValue(char current, char next)
{
	...
	if (Count >= TargetCount - 1)
	{
		...
		{
			if (Execute() == Result::Error) { if (Buf) { delete[] Buf; } return Result::Error; }
			if (Buf) { delete[] Buf; } Buffer.clear();
			Func = &FileReader::ParseIdentifier;
		}
...
Result FileReader::Execute()
{
	...
	/*
	if (Identifier == "PlayerListSize")
	{
		if (InterpretState == None)
		{
			gameWorld->PlayerList.reserve(*(int *)Buf);
			InterpretState = PlayerList;
		}
		else if (InterpretState == EnvironmentList)
		{
			gameWorld->PlayerList.reserve(*(int *)Buf);
		}
	}
	if (Identifier == "EnvironmentListSize")
	{
		gameWorld->EnvironmentList.reserve(*(int *)Buf);
		InterpretState = EnvironmentList;
	}
	*/
...
//Result FileReader::ParseText(char current, char next);