...
Result FileReader::ParseValue(char current, char next)
{
	...
	if (Count >= TargetCount - 1)
	{
		...
		else if (ParseState == 2)
		{
			...
			if (result == Result::End) { return Result::End; }
			Func = &FileReader::ParseIdentifier;
		}
...
Result FileReader::Execute()
{
	...
	if (Identifier == "WorldData")
	{
		ParseOut.push_back(std::to_string(Index));
		return Result::End;
	}
...
Result FileReader::Execute2()
{
	...
	if (Identifier == "Visible")
	{
		...
		{
		case Player:
			GetEnd(gameWorld->PlayerList)->Active = *(bool *)Buf;
			break;
		case Environment:
