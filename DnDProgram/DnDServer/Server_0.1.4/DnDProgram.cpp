...
void DnD::DnDProgram::GraphicWindow::Callback(MessageData message, DnDProgram *program)
{
	...
	if (!Pan || ActiveText || ActiveCharacter)
	{
		...
		if (SelectedTool == Select)
		{
			if (message == WM_LBUTTONDOWN && mousePos.X < 800)
			{
								...
								for (int o = 0; o < GameWorld->EnvironmentList[e]->LayerList[l]->ObjectList.size(); o++)
								{
									if (GameWorld->EnvironmentList[e]->LayerList[l]->ObjectList[o]->Visible)
									{
										...
										if (GameWorld->EnvironmentList[e]->LayerList[l]->ObjectList[o]->ClickedOn(GameWorld->ReverseTransform(mousePos), GameWorld->Scale, WindowPtr->WindowSurface))
										{
											...
											SelectedColour = SelectedObject->ObjectColour;
										}
									}
								}
							}
				...
				for (int i = 0; i < GameWorld->PlayerList.size(); i++)
				{
					...
					{
						SelectPlayer(i);
						SelectedColour = SelectedPlayer->ObjectColour;
					}
				}
			...
			if (SelectedObject && message == WM_LBUTTONDOWN && mousePos.X < 800)
			{
				...
				if (SelectedObject->Type == CharacterObject)
				{
					InitialObjectPos = { ((Character *)SelectedObject)->Pos };
				}
			}
			/*if (message == WM_LBUTTONUP)
			{
			...
			if (SelectedObject && message == WM_MOUSEMOVE && Drag)
			{
				...
				if (SelectedObject->Type == CharacterObject)
				{
					((Character *)SelectedObject)->Pos = InitialObjectPos[0] + (mouseTransform.ToInt2() - InitialMousePos);
				}
				WindowPtr->AddMessage(MessageData({ WL_UPDATESURFACE, 0, 0 }), true);
			}
		...
		{
			SelectedObject->ObjectColour = SelectedColour;
			if (SelectedObject->Type == TextObject)
			{
		...
		if (SelectedTool == SubSelect && SelectedPlayer)
		{
			//SubSelectPlayer(SelectedPlayerIndex);
			SelectedPlayer->ObjectColour = SelectedColour;
			//WindowPtr->AddMessage(MessageData({ WL_UPDATESURFACE, 0, 0 }), true);
		}
		...
		if (SelectedTool == DrawLines && SelectedLayer)
		{
			...
			if (message == WM_LBUTTONDOWN && mousePos.X < 800)
			{
				...
				else
				{
					...
					if (Rect(GameWorld->Transform(ActiveLines->LineList[0]).ToInt2() - 4, GameWorld->Transform(ActiveLines->LineList[0]).ToInt2() + 4).Intersect(mousePos))
					{
		...
		//Character
		if (SelectedTool == PlaceCharacter && SelectedLayer)
		{
			if (message == WM_LBUTTONDOWN && mousePos.X < 800)
			{
				//deselect text
				if (ActiveCharacter)
				{
					//ActiveText->Selected = Unselected;
					DeselectObject(SelectedObjectIndex);
					if (ActiveCharacter->Name.size() < 1) { GameWorld->Remove(ActiveCharacter); }
					//SelectedObject = 0; SelectedObjectIndex = -1; ActiveText = 0;
					ActiveCharacter = 0;
					WindowPtr->AddMessage(MessageData({ WL_UPDATESURFACE, 0, 0 }), true);
				}
				...
				//ActiveText->PlayerList.push_back(GameWorld->PlayerList[0]);
				ActiveCharacter->ObjectColour = SelectedColour;
				ActiveCharacter->Pos = mouseTransform.ToInt2();
				//ActiveText->FontSize = FontSize;
				SelectedLayer->ObjectList.push_back(ActiveCharacter);
				SetObjectContainsPlayers(ActiveCharacter);
				//ActiveText->Selected = SubSelected; SelectedObject = ActiveText; SelectedObjectIndex = (int)SelectedLayer->ObjectList.size() - 1;
				SubSelectObject((int)SelectedLayer->ObjectList.size() - 1);
				Drag = true;
				WindowPtr->AddMessage(MessageData({ WL_UPDATESURFACE, 0, 0 }), true);
		...
		if (ActiveCharacter && (SelectedTool == PlaceCharacter || SelectedTool == SubSelect))
		{
			...
			if (message == WM_KEYDOWN && message.WParam() && ActiveCharacter && !ChangeFontSize)
			{
				char character = (char)message.WParam();
				if (character >= 'A' && character <= 'Z') { ActiveCharacter->Name.push_back(WindowPtr->Input->KeyState[WL_SHIFT].Pressed ? character : character - ('A' - 'a')); }
				if (character >= '0' && character <= '9' || character == ' ') { ActiveCharacter->Name.push_back(character); }
				if (character == WL_BACKSPACE && ActiveCharacter->Name.size()) { ActiveCharacter->Name.pop_back(); }
				WindowPtr->AddMessage(MessageData({ WL_UPDATESURFACE, 0, 0 }), true);
			}
		}
	}
	...
	{/*InterfaceSurface */}
	{
		...
		{
			if (Rect({ 810 - 1, 665 }, { 900, 680 + 1 }).Intersect(mousePos)) { ChangeFontSize = true; WindowPtr->AddMessage(MessageData({ WL_UPDATESURFACE, 0, 0 }), true); }
		...
		if (message == WM_LBUTTONDOWN)
		{
			if (Rect({ 810 - 1, 690 }, { 916, 705 + 1 }).Intersect(mousePos)) { ChangeGridSize = true; WindowPtr->AddMessage(MessageData({ WL_UPDATESURFACE, 0, 0 }), true); }
		...
		if (message == WM_LBUTTONDOWN && Rect({ 920, 690 }, { 935, 705 }).Intersect(mousePos))
		{
			GameWorld->DrawGrid = !GameWorld->DrawGrid;
		}
		if (message == WM_LBUTTONDOWN && Rect({ 980, 690 }, { 995, 705 }).Intersect(mousePos))
		{
		...
		if (message == WM_LBUTTONDOWN)
		{
			...
			if (Rect({ 810, 710 + 15 * 0 }, { 920, 726 + 15 * 0 }).Intersect(mousePos)) { SelectedTool = Select; WindowPtr->AddMessage(MessageData({ WL_UPDATESURFACE, 0, 0 }), true); if (SelectedObject) { SelectedObject->Selected = Selected; } }
			if (Rect({ 810, 710 + 15 * 1 }, { 920, 726 + 15 * 1 }).Intersect(mousePos)) { SelectedTool = SubSelect; WindowPtr->AddMessage(MessageData({ WL_UPDATESURFACE, 0, 0 }), true); if (SelectedObject) { SelectedObject->Selected = SubSelected; } }
			if (Rect({ 810, 710 + 15 * 2 }, { 920, 726 + 15 * 2 }).Intersect(mousePos)) { SelectedTool = DrawLines; WindowPtr->AddMessage(MessageData({ WL_UPDATESURFACE, 0, 0 }), true); }
			if (Rect({ 810, 710 + 15 * 3 }, { 920, 726 + 15 * 3 }).Intersect(mousePos)) { SelectedTool = DrawEllipse; WindowPtr->AddMessage(MessageData({ WL_UPDATESURFACE, 0, 0 }), true); }
			if (Rect({ 810, 710 + 15 * 4 }, { 920, 726 + 15 * 4 }).Intersect(mousePos)) { SelectedTool = PlaceText; WindowPtr->AddMessage(MessageData({ WL_UPDATESURFACE, 0, 0 }), true); }
			if (Rect({ 810, 710 + 15 * 5 }, { 920, 726 + 15 * 5 }).Intersect(mousePos)) { SelectedTool = PlaceCharacter; WindowPtr->AddMessage(MessageData({ WL_UPDATESURFACE, 0, 0 }), true); }
			if (Previous == Select && SelectedTool != Select)
			{
				...
				if (SelectedObject)
				{
					if (SelectedTool == SubSelect)
					{
						...
						if (SelectedObject && SelectedObject->Type == CharacterObject)
						{
							ActiveCharacter = (Character *)SelectedObject;
						}
					}
					else
					{
			...
			if (Previous == SubSelect && SelectedTool != SubSelect)
			{
				...
				if (SelectedPlayer && SelectedPlayer->Selected == SubSelected)
				{
			...
			if (Previous == PlaceCharacter && SelectedTool != PlaceCharacter && ActiveCharacter)
			{
				ActiveCharacter->Selected = Unselected;
				if (ActiveCharacter->Name.size() < 1) { GameWorld->Remove(ActiveCharacter); }
				SelectedObject = 0; SelectedObjectIndex = -1; ActiveCharacter = 0;
				WindowPtr->AddMessage(MessageData({ WL_UPDATESURFACE, 0, 0 }), true);
			}
		}
...
void DnD::DnDProgram::GraphicWindow::DrawSelectMenu(DnDProgram *program)
{
	...
	//Font Select
	InterfaceSurface->DrawText({ 10, 665 }, "FontSize:" + std::to_string(FontSize), Font("consolas", 10, { 255,255,255 }));
	InterfaceSurface->DrawRectangle({ 10 - 1, 665 }, { 100, 680 + 1 }, ChangeFontSize ? Colour(255, 255, 255) : Colour(180, 180, 180), ChangeFontSize ? 2 : 0.5f);


	//Grid Control
	InterfaceSurface->DrawText({ 10, 690 }, "GridSize:" + std::to_string(GameWorld->GridSize), Font("consolas", 10, { 255,255,255 }));
	InterfaceSurface->DrawRectangle({ 10 - 1, 690 }, { 116, 705 + 1 }, ChangeGridSize ? Colour(255, 255, 255) : Colour(180, 180, 180), ChangeGridSize ? 2 : 0.5f);
	if (GameWorld->DrawGrid) { InterfaceSurface->FillRectangle({ 120, 690 }, { 135, 705 }, Colour(255, 255, 255)); }
	else { InterfaceSurface->DrawRectangle({ 120, 690 }, { 135, 705 }, Colour(255, 255, 255)); }
	InterfaceSurface->DrawText({ 145, 690 }, "Snap", Font("consolas", 10, { 255,255,255 }));
	if (SnapToGrid) { InterfaceSurface->FillRectangle({ 180, 690 }, { 195, 705 }, Colour(255, 255, 255)); }
	else { InterfaceSurface->DrawRectangle({ 180, 690 }, { 195, 705 }, Colour(255, 255, 255)); }
	...
	//Tool Selection
	if (SelectedTool == Select) { InterfaceSurface->FillRectangle({ { 10, 712 }, { 120, 724 } }, { 40,100,230 }); InterfaceSurface->DrawText({ 10, 710 }, "Select", Font("consolas", 10, { 255,255,255 })); }
	else { InterfaceSurface->DrawText({ 10, 710 }, "Select", Font("consolas", 10, { 255,255,255 })); }
	
	if (SelectedTool == SubSelect) { InterfaceSurface->FillRectangle({ { 10, 712 + 15 * 1 },{ 120, 724 + 15 * 1 } }, { 40,100,230 }); InterfaceSurface->DrawText({ 10, 725 }, "SubSelect", Font("consolas", 10, { 255,255,255 })); }
	else { InterfaceSurface->DrawText({ 10, 725 }, "SubSelect", Font("consolas", 10, { 255,255,255 })); }
	
	if (SelectedTool == DrawLines) { InterfaceSurface->FillRectangle({ { 10, 712 + 15 * 2 },{ 120, 724 + 15 * 2} }, { 40,100,230 }); InterfaceSurface->DrawText({ 10, 740 }, "DrawLines", Font("consolas", 10, { 255,255,255 })); }
	else { InterfaceSurface->DrawText({ 10, 740 }, "DrawLines", Font("consolas", 10, { 255,255,255 })); }

	if (SelectedTool == DrawEllipse) { InterfaceSurface->FillRectangle({ { 10, 712 + 15 * 3 },{ 120, 724 + 15 * 3} }, { 40,100,230 }); InterfaceSurface->DrawText({ 10, 755 }, "DrawEllipse", Font("consolas", 10, { 255,255,255 })); }
	else { InterfaceSurface->DrawText({ 10, 755 }, "DrawEllipse", Font("consolas", 10, { 255,255,255 })); }

	if (SelectedTool == PlaceText) { InterfaceSurface->FillRectangle({ { 10, 712 + 15 * 4 },{ 120, 724 + 15 * 4 } }, { 40,100,230 }); InterfaceSurface->DrawText({ 10, 770 }, "PlaceText", Font("consolas", 10, { 255,255,255 })); }
	else { InterfaceSurface->DrawText({ 10, 770 }, "PlaceText", Font("consolas", 10, { 255,255,255 })); }

	if (SelectedTool == PlaceCharacter) { InterfaceSurface->FillRectangle({ { 10, 712 + 15 * 5 },{ 120, 724 + 15 * 5 } }, { 40,100,230 }); InterfaceSurface->DrawText({ 10, 785 }, "PlaceCharacter", Font("consolas", 10, { 255,255,255 })); }
	else { InterfaceSurface->DrawText({ 10, 785 }, "PlaceCharacter", Font("consolas", 10, { 255,255,255 })); }
}
...
{
	if (index >= 0 && index < GameWorld->PlayerList.size())
	{
...
void DnD::DnDProgram::GraphicWindow::SubSelectPlayer(int index)
{
	if (index >= 0 && index < GameWorld->PlayerList.size())
	{
		SelectedPlayer = GameWorld->PlayerList[index];
		SelectedPlayer->Selected = SubSelected;
	}
	else
	{
		SelectedPlayer = 0;
...
void DnD::DnDProgram::Run()
{
	...
	//ConsoleWin.CommandLine->Write(std::to_string(HashInt(3,29374)) + "\n");
...
void DnD::DnDProgram::Load(std::string name)
{
	...
	{
		ConsoleWin.CommandLine->Write("ERROR::Error opening file:" + name + "\n");
		GraphicWin.WindowPtr->AddMessage(MessageData({ WL_UPDATESURFACE, 0, 0 }), true);
		return;
	}
	...
	{
		GraphicWin.GameWorld->ClearObjects();
		result = Result::Error;
	}
