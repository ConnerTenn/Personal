...
void DnD::DnDProgram::GraphicWindow::Callback(MessageData message, DnDProgram *program)
{
	...
	{/*InterfaceSurface */}
	{
		...
		if (message == WM_KEYDOWN && message.WParam() && ChangeName)
		{
			...
			{
				program->Name += character - (WindowPtr->Input->KeyState[WL_SHIFT].Pressed ? 0 : ('A'-'a'));
				while (program->Name.size() > 16) { program->Name.pop_back(); }
				WindowPtr->AddMessage(MessageData({ WL_UPDATESURFACE, 0, 0 }), true);
			}
			if (character == WL_BACKSPACE && program->Name.size())
			{
				program->Name.pop_back();
				WindowPtr->AddMessage(MessageData({ WL_UPDATESURFACE, 0, 0 }), true);
			}
		}
...
{
	//Name Select
	InterfaceSurface->DrawText({ 10, 10 }, "Name:" + program->Name, Font("consolas", 12, { 255,255,255 }));
