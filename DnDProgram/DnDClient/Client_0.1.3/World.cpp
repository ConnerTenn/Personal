...
{
	return HashInt(colour.R | colour.G << 8 * 1 | colour.B << 8 * 2 | colour.A << 8 * 3, hash);

}

uint32_t DnD::HashInt(uint32_t val, uint32_t hash)
{
	return hash ^ (~val);//((val << 16 | val >> 16));
}

uint32_t DnD::HashLong(uint64_t val, uint32_t hash)
{
	return HashInt((uint32_t)val, hash) ^ HashInt((uint32_t)(val << 32), hash);
}
...
{
	return HashInt(val.Y, HashInt(val.X, hash));
}

uint32_t DnD::HashDouble2(Double2 val, uint32_t hash)
{
	return HashLong((uint64_t)val.X, hash) ^ HashLong((uint64_t)val.Y, hash);
}
...
int DnD::WorldObject::LoadFromData(char *data, WorldObjectTypes type)
{
	int offset = 0;
...
int DnD::Lines::LoadFromData(char *data, WorldObjectTypes type)
{
	int offset = 0;

	offset += WorldObject::LoadFromData(data + offset, type);
...
int DnD::Ellipse::LoadFromData(char *data, WorldObjectTypes type)
{
	int offset = 0;
...
void DnD::Text::Draw(Surface *surface, World *world)
{
	if (Visible)
	{
		...
		if (Selected == SubSelected)
		{
			Int2 dimensions = MeasureString(surface, TextData.size() ? TextData : "a", Font("consolas", float(float(FontSize) * world->Scale), ObjectColour, Selected ? WL_UNDERLINE : 0));
...
int DnD::Text::LoadFromData(char *data, WorldObjectTypes type)
{
	int offset = 0;
...
void DnD::Character::Draw(Surface *surface, World *world)
{
	if (Visible)// && ContainsPlayer(activePlayer))//draw)
	{
		...
		surface->DrawEllipse(pos - dimensions, pos + dimensions, ObjectColour, Selected ? 3 : 0.5f);
		surface->FillEllipse(pos - dimensions, pos + dimensions, Colour(ObjectColour.R, ObjectColour.G, ObjectColour.B, 150));

		dimensions = MeasureString(surface, Name, Font("consolas", 7, { 255,255,255 }));
		surface->DrawText(world->Transform(Pos - dimensions / 2).ToInt2(), Name, Font("consolas", float(7 * world->Scale), { 255,255,255 }, Selected == SubSelected ? WL_UNDERLINE : 0));
...
int DnD::Character::LoadFromData(char *data, WorldObjectTypes type)
{
	int offset = 0;
...
{
	for (int i = 0; Visible && i < ObjectList.size(); i++)
	{
...
int DnD::Layer::LoadFromData(char *data)
{
	int offset = 0;
	...
	for (int i = 0; i < size; i++)
	{
		...
		else if (type == CharacterObject) { worldObject = new Character(); }
...
{
	for (int i = 0; Visible && i < LayerList.size(); i++)
	{
...
int DnD::Environment::LoadFromData(char *data)
{
	int offset = 0;
	...
	for (int i = 0; i < size; i++)
	{
		...
		offset += layer->LoadFromData(data + offset);
...
void DnD::World::Draw(Environment *activeEnvironment)
{
	//for (int i = 0; i < EnvironmentList.size(); i++)
	//{
	//	EnvironmentList[i]->Draw(ParentWindow->WindowSurface, this);
	//}
	if (activeEnvironment) { activeEnvironment->Draw(ParentWindow->WindowSurface, this); }

	{
		Int2 pos = Transform(Position).ToInt2();
		Int2 dimensions = (Double2(10 * Scale, 10 * Scale)).ToInt2();
		ParentWindow->WindowSurface->DrawEllipse(pos - dimensions, pos + dimensions, SelfColour, 3);
		dimensions = (Double2(max(5, 10 * Scale), max(5, 10 * Scale))).ToInt2();
		ParentWindow->WindowSurface->FillEllipse(pos - dimensions, pos + dimensions, { SelfColour.R,SelfColour.G,SelfColour.B,150 });

		dimensions = MeasureString(ParentWindow->WindowSurface, Name, Font("consolas", 7, { 255,255,255 }));
		ParentWindow->WindowSurface->DrawText(Transform(Position - dimensions / 2).ToInt2(), Name, Font("consolas", float(7 * Scale), { 255,255,255 }, WL_UNDERLINE));
	}
