...
void NetworkManager::Receive(int eventIndex, std::string identifier, char *data, int size)
{
	...
	{
		int offset = 0;
		Program->GraphicWin.GameWorld->Position = *(Int2 *)(data + offset); offset += sizeof(Int2);
		Program->GraphicWin.GameWorld->SelfColour = *(Colour *)(data + offset); offset += sizeof(Colour);
